...
const CONSUME_AMOUNT = &'consume_amount'
const IS_CONSUMED = &'is_consumed'
...
func set_item_group_name(group_name: StringName):
	if _item_group_name != group_name:
		if _item_group_name != Items.NULL_GROUP_NAME:
			remove_from_group(_item_group_name)
	_item_group_name = group_name
	if _item_group_name != Items.NULL_GROUP_NAME:
...
func consume(_delta: float) -> Dictionary:
	despawn()
	return _make_consume_ret(1.0, true)

func _make_consume_ret(amount: float, is_consumed: bool) -> Dictionary:
	return {CONSUME_AMOUNT: amount, IS_CONSUMED: is_consumed}
